...
## 12. Final Revenue Single-Source Rule and Forecast Controls (2026-08-12)

### Non-negotiable revenue source of truth

* Use **only** `buildTrendData()` (`lib/trends.ts`) for rendered cumulative revenue, period revenue, Revenue Trends KPI cards, Revenue by WBS & Period, summary Revenue/POC/revenue-split widgets, and Forecast & Completion inputs.
* The ledger inputs are Historical Revenue pre-2026 + sign-reversed GR55 revenue postings from 2026 onward + current/latest data-period POC accrual. Bucket every source by **Posting Date**.
* The prior implementation incorrectly had KPI cards summing saved `revenue_wbs.recognized_revenue_to_date`, while the WBS matrix independently used the ledger. This produced SAR 14,906,683.85 vs SAR 14,928,759.70 for SEC NG NMFOC. The cards now consume `latestTrendPoint.cumulativeRecognizedRevenue` and in-month revenue consumes `latestTrendPoint.recognizedRevenue`.
* Never reintroduce a dashboard total from `revenue_wbs.recognized_revenue_to_date`, `opening_recognized_revenue`, `mtd_revenue_recognition`, or `sap_earned_revenue`. Those remain persisted compatibility/snapshot fields only.
* The exact reference figures for the current SEC NG NMFOC source data are: cumulative ledger revenue **SAR 14,928,759.70**, August 2026 ledger/P0C accrual **SAR 223,876.57**, planned revenue **SAR 36,266,300.64**.

### Forecast mechanics currently implemented

* Forecast uses the same ledger cumulative revenue for remaining revenue: `planned revenue - ledger cumulative recognized revenue`.
* Revenue bases include `last1`, `last2`, `last3` (default), `ytd`, `recommended`, `targetDate`, and `monthlyTarget`.
* Recommended basis is deterministic: uses latest month if the last three completed months materially increase/decrease; otherwise last-three-month average. It displays its reason, trend, stability, and confidence.
* Target Completion Date derives the required revenue run rate from remaining revenue divided by the exact remaining partial/current-month and future-month capacity. Monthly Revenue Target accepts user-entered target revenue. Both compare against the actual last-three-month run rate and show status/gap/improvement.
* Cost forecast always uses the eligible project cost-to-revenue ratio from the selected historical basis. Cost Element Control remains mandatory wherever GR55 cost contributes. Revenue GLs `400110`, `400119`, `400210`, `400310` are never cost.
* Basis and scenario tables must visibly mark the selected method. For target-based modes, include the target as a selected row. Scenario final cost can remain the same across scenarios because the same remaining contract revenue and cost/revenue ratio are recognised; timing and cash-flow profile change.
