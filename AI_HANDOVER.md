...
* **Storage** — `lib/dashboard-layout.ts`: one server-side JSON file `.local-db/dashboard-layout.json`. Shape now `{ global:{<id>:status}, projects:{<pid>:{<id>:status}}, order:{ global:{<tab>:[ids]}, projects:{<pid>:{<tab>:[ids]}} } }`. Mirrors `runtime-config.ts` (no settings table). **Backward-compatible**: legacy flat and no-`order` files load cleanly (order defaults to empty). Status effective = `global` merged with project override (project wins). Order effective = project order ?? global order ?? registry default, **all-or-nothing per level** (not merged), always **completed** (`completeOrder` appends any registry ids missing from a saved order, so new widgets never disappear). Order writes drop a redundant override (order equal to the fallback → key deleted), same as status.
* **Reorder UI** — `@dnd-kit` (core/sortable/modifiers/utilities). `components/dashboard-grid.tsx` (`DashboardGrid`) is the reusable renderer: **view mode** = a plain `grid grid-cols-12` (zero dnd overhead); **edit mode** = `DndContext`+`SortableContext` (`rectSortingStrategy`) with an explicit **drag handle** per cell (handle-only listeners so interactive widget content isn't hijacked; 6px pointer activation), a `DragOverlay` preview, and **lightweight placeholders swapped in for heavy charts/table during drag**. Reorder emits the new *visible* id sequence; the parent merges it back into the full order (hidden ids keep their slots). The full order (incl. hidden) is what's persisted, so hide→show restores position.
* **Scope = global default + per-project overrides** for both axes.
  ...
  * Order: per-project via the **"Edit layout" button** in the dashboard tab bar (Admin-only, Summary tab) → drag → Save posts `{ order, tab }` to `/api/dashboard-layout/[projectId]` → reload.
* **Both APIs** (`/api/settings/dashboard-layout`, `/api/dashboard-layout/[projectId]`) now accept/return **both** `{ layout }` and `{ order, tab }` (independent axes, one file), POST Admin-gated.
* **Render path** — `page.tsx` resolves `getEffectiveDashboardLayout` + `getEffectiveOrder(projectId,'summary'|'trends')` server-side (correct SSR, no order flash) and passes `dashboardLayout` + `summaryOrder` + `trendsOrder` to `DashboardClientWorkspace`. The **Summary tab is data-driven**: each visual is a case in `renderSummaryWidget(id)`, filtered by `isSummaryVisible(id)`, rendered via `DashboardGrid` in `summaryOrder`. Default (no file) = everything visible in registry order → the system is invisible until an Admin changes something.
